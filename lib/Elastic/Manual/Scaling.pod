...
Start a second node and ElasticSearch will bring up the 5 replica shards. Now,
...
with a new index name:
    ...
    my $new_index = 'myapp_'.time();
    $ns->index($new_index)->create;

Now you can reindex your data from C<$index> to C<$new_index>, and finally
update the alias and delete the old index:
...
Add it to the main alias for the namespace, C<myapp>:
    ...
    $ns->alias->add_index('myapp_2012_06');
...
Next month, you can update the C<last_two_months> alias with:
    ...
    $last_2->remove_index('myapp_2013_01');
    $last_2->add_index('myapp_2013_01');
...
a big index:
    ...
    my $index = 'twitter_'.time();
    $ns->index($index)->create( settings => { number_of_shards => 100 });

Then we add that index to our main domain C<myapp>, so that Elastic::Model
knows that it uses the same namespace:

    $ns->alias->add_index($index);

Once we have reindexed the existing data from the old C<twitter> alias to
the new index, we update the alias to point to the new index:
