...
which works pretty much like any iterator, eg:
    ...
    $it->pop;           # return first element and remove it from $it
...
But that is verbose. By default, L<first()>, L<next()> etc all return Result
...
to do that. Instead of using C<next()> you should use C<pop()>:

    while ( my $result = $results->pop ) {
...
When using C<pop()>, while the L<Elastic::Model::Results::Scrolled/size>
attribute starts out the same as the L<Elastic::Model::Results::Scrolled/total>
attribute, it will decrement by one for each C<pop()> call.
